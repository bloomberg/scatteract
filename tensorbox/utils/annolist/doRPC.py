...
def comp_prec_recall(annoIDL, detIDL, minOverlap):
        ...
        precs, recalls, scores, fppi, graphs, accuracy = comp_prec_recall_all_params(annoIDL, detIDL, ignoreIDL, minOverlap=minOverlap);
...
def comp_prec_recall_all_params(annoIDL, detIDL, ignoreIDL, minWidth=0, minHeight=0, maxWidth=float('inf'), maxHeight=float('inf'), 
	...
	for i,nextrect in enumerate(allRects):
		...
		if (i == numDet - 1 or score != allRects[i + 1].rect.score or firstFP or i == len(allRects)):	
			if(changed or i == numDet - 1 or i == len(allRects)):
				...
				if(lastcd > 0):
					...
					accuracy.append(float(lastsf)/(float(noAnnotations - lastiflow+lastcd - lastsf)))     
				...
				if (cd > 0):
					...
					accuracy.append(float(sf)/(float(noAnnotations - iflow+cd -sf)))  
        ...
        return precs, recalls, scores, fppi, graphs, accuracy
...
def main():
	...
	if (options.dropFirst == True):
        ...
        verbose = True;
        precs, recalls, scores, fppi, graphs, accuracy = comp_prec_recall_all_params(annoIDL, detIDL, ignoreIDL,
	...
	for i in xrange(len(precs)):
		file.write(str(precs[i])+" "+str(recalls[i])+" "+str(scores[i])+ " " + str(fppi[i])+ " " + str(accuracy[i])+  "\n")
	file.close()
